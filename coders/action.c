...
/*   Updated: 2026/04/21 09:57:30 by flauweri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	print(coder->global, get_time_ms(), coder->name, "is debugging");
	usleep(coder->global->config.time_to_debug * 1000);
}	

void is_refactoring(t_coder *coder)
{
	print(coder->global, get_time_ms(), coder->name, "is refactoring");
	usleep(coder->global->config.time_to_refactor * 1000);
}

void is_compiling(t_coder *coder)
{
	print(coder->global, get_time_ms(), coder->name, "is compiling");
	coder->last_compile_start = get_time_ms() - coder->global->start;
...
void try_to_take(t_coder *coder, t_dongle *dongle)
{
	...
	print(coder->global, get_time_ms(), coder->name, "has taken a dongle");
}
